...
* Basic Babel ES6 (does not include babel-polyfill), Object Rest/Spread Properties, fetch, promises, Object.assign, maps, sets and Array.from. This list can be easily extended.
...
1. `git clone https://github.com/AndrewBanks10/create-react-project` or download zip.
2. Go to your command window at the root of where you copied the create-react-project and type 
`npm run createproject` 
The installation will take a few minutes. Once npm install has been run for you, the project configuration UI will be displayed. This UI allows you to configure your project and also create react components. Select your project configuration options and save. You can also create react component in the UI program.
3. Open the folder in which you installled the files for create-react-project in Visual Studio Code.
4. Make sure you have [vscode-chrome-debug](https://marketplace.visualstudio.com/items?itemName=msjsdiag.debugger-for-chrome) and [vscode-eslint](https://marketplace.visualstudio.com/items?itemName=dbaeumer.vscode-eslint) extension installed
...
```
### How to Change State in an MVC Component from External Code
If you need to change component's state based on some changes that happen elsewhere in the app, define an exported function in the target Component as below. This may be needed for example as data comes in from a webSocket. It is recommended that only the owner of the state partition performs changes on that component's state. 
```javascript
export doStateChange = (key, value) => {
    partitionState[key] = value
}
```
Then import and export the function in the component index file. Then simply import the function into the code that needs to change the component's state.
```javascript
import { doStateChange } from '../Component2'

doStateChange( 'counter', 2 )
```
### How to Access State in an MVC Component from External Code
Assume you have a login component and you need to access some user features in that component from another module. In this case, the target controller should return a shallow copy of the the required property. Now, keep in mind that the copy may contain properties that contain objects. This would allow the requesting module to alter those embedded objects which would not be picked by the create-react-project monitoring component. So, you may want to consider returning a deep copy of the object. However, if the object to be copied is complex then the deep copy operation can be very slow. In any event, these are the tradeoffs to consider. See the example below.
```javascript
// Component2 controller. This function must be imported into
// index and then exported also.
export userOption1 = () =>
    (partitionState.user.option1)

// Component1 controller.
import { userOption1 } from '../Component2'
...
const { partitionStore, partitionState, getState, setState, subscribe, wrappedComponents } = establishControllerConnections({
  module,
  partition: { partitionName: counterPartition, defaultState, controllerFunctions },
  storeKeys: ['counter']
  uiComponent: Counter,
...
### How to Access Controller Functions for Unit Testing
The partitionStore object that is exported in the component index file contains all of the controller functions. So, in your component test code you can do something like the below.
```javascript
...
Causality-redux provides a global store partition by default in create-react-project. See the below for an example on how to use this global store partition.
```javascript
...
Simply install the library that you need and then import it into src/bootstrap/libs. Also list the additional library using the configuration program in the dll library section so that debug compilation is faster.
...
Download the create-react-project project files into a directory named counterdemo. Run the command below from the command line in the counterdemo directory.
```
npm run createproject
```
...
```
MainApp is always the root of the react tree with create-react-project and it inherits all the necessary providers based on the project configuration.
...
Note that create-react-project does all the grunt work for you in terms of setting up the project such that you need only write javascript business code, react jsx stateless components, design the redux partition for the component with defaultState and finally write javascript controller functions to call the business code and set redux partition values so that the react UI properly renders the updated results. Finally, you can optionally add mocha test code without having to perform any setup. Hence, create-react-project provides an optimal way to write MVC stateless react components.

## Simplicity with Programming React Components Using Create-react-project
To create react components using create-react-project, you only need to program the five simple items listed above.
1. Define your component state variables under defaultState.
2. Define your component service functions in controllerFunctions.
