...
var CausalityRedux = function () {
        ...
        return function (listener, stateEntries) {
            ...
            stateEntries.forEach(function (se) {
                ...
                if (_typeof(partition.defaultState[se]) !== undefinedString) found = true;else {
                    Object.keys(partition.changerDefinitions).forEach(function (key) {
                        if (key === se) {
    ...
    function setupPartition(store, stateEntry) {
        ...
        Object.keys(stateEntry.changers).forEach(function (o) {
    ...
    var buildStateEntryChangersAndReducers = function buildStateEntryChangersAndReducers(entry) {
        ...
        var validateChangerArg = function validateChangerArg(changerArg) {
            Object.keys(changerArg).forEach(function (tag) {
        ...
        //
        var userDefinedReducer = function userDefinedReducer(reducer) {
            return function (state, action) {
        ...
        //
        Object.keys(entry.changerDefinitions).forEach(function (o) {
    ...
    function validatePartition(stateEntry) {
        var changerKeys = [];
        if (_typeof(stateEntry.changerDefinitions) !== undefinedString) changerKeys = Object.keys(stateEntry.changerDefinitions);
        if (_typeof(stateEntry.changers) !== undefinedString) changerKeys = [].concat(_toConsumableArray(changerKeys), _toConsumableArray(Object.keys(stateEntry.changers)));
        ...
        Object.keys(stateEntry).forEach(function (o) {
    ...
    function init(partitionDefinitions, preloadedState, enhancer) {
        ...
        var generalReducer = function generalReducer() {
            ...
            if (!_startState) _startState = state;
        ...
        var generalListener = function generalListener() {
                        ...
                        if (item.stateEntries.length === 0) {
                            ...
                            if (_typeof(item.prevState) === undefinedString) {
                                item.stateEntries.forEach(function (se) {
                                    areEqual = areEqual && state[_o][se] === _startState[_o][se];
                                });
                            } else {
                                item.stateEntries.forEach(function (entry) {
                                    areEqual = areEqual && state[_o][entry] === item.prevState[entry];
                                });
                            }
                            ...
                            if (!areEqual) {
                                var nextState = {};
                                item.stateEntries.forEach(function (entry) {
                                    nextState[entry] = state[_o][entry];
                                });
                                item.prevState = nextState;

                                indicateListener(_o, nextState, item.listenerName);
            ...
            for (var _o in _partitionsThatChanged) {
                _loop(_o);
            }
